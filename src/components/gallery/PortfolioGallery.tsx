...
export function PortfolioGallery({ collections, preview = false }: { collections: PortfolioCollection[]; preview?: boolean }) {
  const [active, setActive] = useState<string | null>(preview ? null : "all");
    ...
    return (
      <>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {displayCollections.map((collection, index) => collection.images[0] && (
            <article key={collection.id} className={`image-sheen lift-card group relative overflow-hidden rounded-2xl ${index === 0 ? "sm:col-span-2" : ""}`}>
              <div className="relative aspect-[4/3]"><Image src={collection.images[0].src} alt={collection.images[0].alt} fill priority={index < 2} className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" /></div>
